...
class ImageHandler(object):
	...
	def __init__(self, dir_path, h, w, fonts):
		...
		self.blurred = cv2.GaussianBlur(self.gray, (self.blur_r, self.blur_r), 0)
	...
	def check_active(self, mouse):
		...
		for slider in self.sliders:
			...
			if self.sliders[slider].active:
				...
				if self.phase == "edge":
					...
					self.START_R, self.END_R = 1 + int(14 * (100 - self.sliders["max1"].percent) / 100), 1 + int(12 * (100 - self.sliders["min1"].percent) / 100)
					self.sliders["min1"].update(set_min=min(100, (( 100 - (self.START_R - 1) * 100.0 / 14) + 2*SCALE())))
					self.sliders["max1"].update(set_max=max(0, (( 100 - (self.END_R - 1) * 100.0 / 14) - 2*SCALE())))

					self.JUMP_LENGTH = rn(self.max_jump * (100 - self.sliders["max2"].percent) / 100)
		...
		if self.buttons["auto"].active:
			...
			if self.phase == "edge":
				...
				self.sliders["min1"].update(percent = 100 - (self.START_R - 1) * 100.0 / 14) 
				self.sliders["max1"].update(percent = 100 - (self.END_R - 1) * 100.0 / 14) 
				...
				while len(self.trails) > 0:
					...
					print next_start
					print self.trails[next_start][0]
					#pause()
...
def find_direction(a, x, y, r, threshold):
  ...
  cellstarts = [[x1-rn(d*_RT2), y1-rn(d*_RT2)], [x1-d, y1], [x1-rn(d*_RT2), y1+rn(d*_RT2)],
                    		 [x1, y1-d],             					  		[x1, y1+d],
                [x1+rn(d*_RT2), y1-rn(d*_RT2)], [x1+d, y1], [x1+rn(d*_RT2), y1+rn(d*_RT2)],

                [x1-rn(d*RT2_3), y1-rn(d*0.5)], [x1-rn(d*0.5), y1-rn(d*RT2_3)],
                [x1+rn(d*RT2_3), y1-rn(d*0.5)], [x1+rn(d*0.5), y1-rn(d*RT2_3)],
                [x1-rn(d*RT2_3), y1+rn(d*0.5)], [x1-rn(d*0.5), y1+rn(d*RT2_3)],
                [x1-rn(d*RT2_3), y1+rn(d*0.5)], [x1-rn(d*0.5), y1+rn(d*RT2_3)],

                [x1-rn(d*C22_5), y1-rn(d*S22_5)], [x1-rn(d*S22_5), y1-rn(d*C22_5)],
                [x1+rn(d*C22_5), y1-rn(d*S22_5)], [x1+rn(d*S22_5), y1-rn(d*C22_5)],
                [x1-rn(d*C22_5), y1+rn(d*S22_5)], [x1-rn(d*S22_5), y1+rn(d*C22_5)],
                [x1-rn(d*C22_5), y1+rn(d*S22_5)], [x1-rn(d*S22_5), y1+rn(d*C22_5)]]
